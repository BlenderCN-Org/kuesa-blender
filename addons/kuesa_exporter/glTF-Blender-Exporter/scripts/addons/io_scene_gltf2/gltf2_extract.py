...
def extract_primitives(glTF, blender_mesh, blender_vertex_groups, export_settings):
    ...
    for blender_polygon in blender_mesh.polygons:
        ...
        # Primitive type
        face_wireframe_enabled = any([
            ext['extension'].ext_meta['name'] == 'KDAB_Kuesa_FaceWireframe'
            and getattr(ext['settings'], 'export_face_wireframe') is True
            for ext in export_settings['gltf_extensions']
        ])
        primitive['mode'] = 1 if face_wireframe_enabled else 4  # Line if wireframe, Triangle otherwise

        loop_indices_count = len(blender_polygon.loop_indices)
        if face_wireframe_enabled and loop_indices_count > 1:
